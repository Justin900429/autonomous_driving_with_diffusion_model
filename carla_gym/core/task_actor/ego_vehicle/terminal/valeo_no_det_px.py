...
class ValeoNoDetPx(object):
    ...
    def get(self, timestamp):
        ...
        # Done condition 7: leave too far from the next waypoint
        c_leave_target = False
        next_waypoint = self._ego_vehicle.get_next_location[0].transform.location
        d_vec = next_waypoint - ev_loc
        if self.prev_next_waypoint is None:
            self.prev_next_waypoint = next_waypoint
            self.prev_distance = np.linalg.norm(np.array([d_vec.x, d_vec.y]))
        elif self.prev_next_waypoint is not None:
            diff_with_prev_waypoint = self.prev_next_waypoint - next_waypoint
            distance = np.linalg.norm(np.array([d_vec.x, d_vec.y]))
            if np.isclose(
                np.array([diff_with_prev_waypoint.x, diff_with_prev_waypoint.y]), 0
            ).all():
                if distance > self.prev_distance + 0.1:
                    c_leave_target = True
                    self.prev_next_waypoint = None
                    self.prev_distance = None
                else:
