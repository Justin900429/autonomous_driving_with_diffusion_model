...
from carla_gym.core.obs_manager.object_finder.pedestrian import (
    ObsManager as OmPedestrian,
)
...
class ValeoAction(object):
    def __init__(self, ego_vehicle):
        ...
        self.om_vehicle = OmVehicle(
            {"max_detection_number": 10, "distance_threshold": 15}
        )
        self.om_pedestrian = OmPedestrian(
            {"max_detection_number": 10, "distance_threshold": 15}
        )
    ...
    def get(self, terminal_reward):
        ...
        desired_spd_veh = (
            desired_spd_ped
        ) = desired_spd_rl = desired_spd_stop = self._maxium_speed
        ...
        if (
            ...
            or light_state == carla.TrafficLightState.Yellow
        ):
        ...
        if (stop_sign is not None) and (
            not self._ego_vehicle.criteria_stop._stop_completed
        ):
        ...
        if hazard_vehicle_loc is None:
            ...
            txt_hazard_veh = np.array2string(
                hazard_vehicle_loc[0:2], precision=1, separator=",", suppress_small=True
            )
        if hazard_ped_loc is None:
            txt_hazard_ped = "[]"
        else:
            txt_hazard_ped = np.array2string(
                hazard_ped_loc[0:2], precision=1, separator=",", suppress_small=True
            )
        if light_loc is None:
            txt_light = "[]"
        else:
            txt_light = np.array2string(
                light_loc[0:2], precision=1, separator=",", suppress_small=True
            )
        if stop_loc is None:
            txt_stop = "[]"
        else:
            txt_stop = np.array2string(
                stop_loc[0:2], precision=1, separator=",", suppress_small=True
            )
