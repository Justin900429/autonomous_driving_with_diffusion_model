...
class EgoVehicleHandler(object):
    ...
    def _filter_spawn_points(self, ev_spawn_locations):
        ...
        def proximity_to_ev(transform):
            return any(
                [
                    ev_loc.distance(transform.location) < self._spawn_distance_to_ev
    ...
    def reset(self, task_config):
        ...
        for ev_id in actor_config:
            ...
            # Set the spectator to the first ego vehicle
            if not set_camera:
                set_camera = True
                ego_location = carla_vehicle.get_location()
                camera_position = carla.Transform(
                    ego_location + carla.Location(z=40), carla.Rotation(pitch=-90)
                )
                self._world.get_spectator().set_transform(camera_position)

                global_route = self.ego_vehicles[ev_id]._global_route
    ...
    def apply_control(self, control_dict=None):
        ...
        for ev_id, control in control_dict.items():
            ...
            if not set_camera:
                set_camera = True
                ego_location = self.ego_vehicles[ev_id].vehicle.get_location()
                camera_position = carla.Transform(
                    ego_location + carla.Location(z=40), carla.Rotation(pitch=-90)
                )
                self._world.get_spectator().set_transform(camera_position)
