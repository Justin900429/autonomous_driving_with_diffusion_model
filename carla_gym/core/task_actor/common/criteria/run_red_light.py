    ...
    def tick(self, vehicle, timestamp):
        ...
        for idx_tl in range(TrafficLightHandler.num_tl):
            ...
            for idx_wp in range(len(TrafficLightHandler.list_stopline_wps[idx_tl])):
                ...
                dot_ve_wp = (
                    ev_dir.x * wp_dir.x + ev_dir.y * wp_dir.y + ev_dir.z * wp_dir.z
                )
                ...
                if (
                    ...
                    (
                        stop_left_loc,
                        stop_right_loc,
                    ) = TrafficLightHandler.list_stopline_vtx[idx_tl][idx_wp]
                    # Is the vehicle traversing the stop line?
                    if self._is_vehicle_crossing_line(
                        (tail_close_pt, tail_far_pt), (stop_left_loc, stop_right_loc)
                    ):
