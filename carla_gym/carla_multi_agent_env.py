...
class CarlaMultiAgentEnv(gym.Env):
    def __init__(
        ...
        self._ev_handler = EgoVehicleHandler(
            self._client, reward_configs, terminal_configs, self._tm
        )
    ...
    def reset(self):
        ...
        cur_waypoints = self._ev_handler.get_current_pos()
        target_waypoints = self._ev_handler.get_target_pos()
        next_waypoints = self._ev_handler.get_next_pos()
        for ev_id in self._om_handler._obs_managers.keys():
