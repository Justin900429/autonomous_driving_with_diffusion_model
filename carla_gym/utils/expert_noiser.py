...
class ExpertNoiser(object):
    ...
    def __init__(
        ...
        self.noise_time_amount = min_noise_time_amount + float(
    ...
    def get_noise_removing(self):
        # print 'REMOVING'
        added_noise = (
            (self.noise_end_time - self.noise_start_time) * 0.02 * self.intensity
        )
        # print added_noise
        if self.noise_type == "Spike" or self.noise_type == "Throttle":
            if self.current_noise_mean > 0:
                ...
                return (
                    added_noise
                    - (time.time() - self.noise_end_time) * 0.03 * self.intensity
                )
            else:
    ...
    def is_time_for_noise(self, steer):
        ...
        if self.remove_noise:
            ...
            if (time.time() - self.noise_end_time) > (
                self.noise_time_amount
            ):  # if half the amount put passed
                self.remove_noise = False
            ...
            if self.is_time_for_noise(action.steer):
                ...
                if self.remove_noise:
                    steer_noisy = max(
                        min(
                            steer
                            + self.get_noise_removing() * (25 / (2.3 * speed + 5)),
                            1,
                        ),
                        -1,
                    )
                    ...
                    steer_noisy = max(
                        min(steer + self.get_noise() * (25 / (2.3 * speed + 5)), 1), -1
                    )
