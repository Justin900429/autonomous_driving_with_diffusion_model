...
def lbc_hazard_vehicle(
    obs_surrounding_vehicles, ev_speed=None, proximity_threshold=9.5
):
    for i, is_valid in enumerate(obs_surrounding_vehicles["binary_mask"]):
        ...
        with_distance_ahead = is_within_distance_ahead(
            sv_loc, proximity_threshold, up_angle_th=45
        )
...
def lbc_hazard_walker(
    obs_surrounding_pedestrians, ev_speed=None, proximity_threshold=9.5
):
...
def behavior_hazard_vehicle(
    ...
    for i, is_valid in enumerate(actors["binary_mask"]):
        ...
        if not at_junction and (
            ...
            if (
                actors["road_id"][i] != next_road_id
                or actors["lane_id"][i] != next_lane_id + lane_offset
            ):
                continue
