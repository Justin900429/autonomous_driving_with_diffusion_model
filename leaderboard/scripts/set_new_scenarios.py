    ...
    if scenarios["scenario_type"] in args.scenarios:
        ...
        for i in range(len(event_list)):
            ...
            location = carla.Location(
                float(_waypoint["x"]), float(_waypoint["y"]), float(_waypoint["z"])
            )
            ...
            if args.debug:
                ...
                spectator.set_transform(
                    carla.Transform(
                        location + carla.Location(z=50), carla.Rotation(pitch=-90)
                    )
                )
                print(
                    " Scenario [{}/{}]. Press Enter for the next scenario".format(
                        i + 1, len(event_list)
                    )
                )
                input()
    ...
    if scenarios["scenario_type"] in args.scenarios:
        ...
        for i in range(len(event_list)):
            ...
            rotation = carla.Rotation(
                float(0), float(_waypoint["pitch"]), float(_waypoint["yaw"])
            )
            world.debug.draw_point(
                location, size=float(0.15), color=carla.Color(0, 255, 255)
            )
            world.debug.draw_string(
