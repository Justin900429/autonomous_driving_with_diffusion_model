...
class ScenarioRunner(object):
    ...
    def __init__(self, args):
        ...
        if LooseVersion(dist.version) < LooseVersion("0.9.8"):
            raise ImportError(
                "CARLA version 0.9.8 or newer required. CARLA version found: {}".format(
                    dist
                )
            )
    ...
    def _analyze_scenario(self, config):
        ...
        if not self.manager.analyze_scenario(
            self._args.output, filename, junit_filename
        ):
...
def main():
    ...
    parser.add_argument(
        "--port", default="2000", help="port to listen to (default: 2000)"
    )
    parser.add_argument(
        "--timeout",
        default="10.0",
