...
class VAAgent(autonomous_agent.AutonomousAgent):
    ...
    def run_step(self, input_data, timestamp):
        ...
        cmd_one_hot = (
            torch.tensor(cmd_one_hot).view(1, 6).to("cuda", dtype=torch.float32)
        )
        speed = (
            torch.FloatTensor([float(tick_data["speed"])])
            .view(1, 1)
            .to("cuda", dtype=torch.float32)
        )
        speed = speed / 12
        rgb = (
            self._im_transform(tick_data["rgb"])
            .unsqueeze(0)
            .to("cuda", dtype=torch.float32)
        )
        ...
        steer_ctrl, throttle_ctrl, brake_ctrl, metadata = self.net.process_action(
    ...
    def save(self, tick_data):
        frame = self.step // 10
        for color, key in zip(COLOR_LIST, ["wp_1", "wp_2", "wp_3", "wp_4"]):
            waypoint = self.pid_metadata[key]
            assert len(waypoint) == 2
            pixel_x = way_point_to_pixel(-waypoint[0])
            pixel_y = way_point_to_pixel(waypoint[1])
            tick_data["bev"] = cv2.circle(
                tick_data["bev"], (int(pixel_x), int(pixel_y)), 3, color, -1
            )
