...
def img_augment_func(image_iteration):
    iteration = image_iteration / 32
    frequency_factor = min(0.05 + float(iteration) / 200000.0, 0.5)
    color_factor = min(float(iteration) / 1000000.0, 0.5)
    dropout_factor = 0.198667 + (0.03856658 - 0.198667) / (
        1 + (iteration / 196416.6) ** 1.863486
    )

    blur_factor = min(0.5 + (0.5 * iteration / 100000.0), 0.5)
