...
def report_dataset_size(dataset_dir):
    ...
    log.warning(
        f"{dataset_dir}: {len(list_h5_path)} episodes, "
...
class DataWriter:
    ...
    def write(self, timestamp, obs, supervision, reward, control_diff=None):
        ...
        action_str = np.array2string(
            supervision[self._ev_id]["action"],
            precision=2,
            separator=",",
            suppress_small=True,
        )
    ...
    def close(
        self, terminal_debug, dagger_thresholds, remove_final_steps, last_value=None
    ):
        # clean up data
        ...
        if self._data_list[0]["control_diff"] is not None:
            ...
            def is_critical(x):
                c_t = (dagger_thresholds.throttle is not None) and (
                    x[0] > dagger_thresholds.throttle
                )
                c_s = (dagger_thresholds.steer is not None) and (
                    x[1] > dagger_thresholds.steer
                )
                c_b = (dagger_thresholds.brake is not None) and (
                    x[2] > dagger_thresholds.brake
                )
                c_a = (dagger_thresholds.acc is not None) and (
                    x[0] + x[2] > dagger_thresholds.acc
                )
            ...
            for idx, data in enumerate(self._data_list):
                if not is_critical(data["control_diff"]):
                    ...
                    n_critical_frames -= 1
            log.warning(
                f"Critical sampling finished, n_critical_frames={n_critical_frames}"
            )
        else:
            ...
            if remove_final_steps:
                if terminal_debug["traffic_rule_violated"]:
                    ...
                    if len(self._data_list) < 300:
                        valid = False
                    log.warning(
                        f"traffic_rule_violated, valid={valid}, len={len(self._data_list)}"
                    )
        ...
        if valid:
            ...
            log.info(
                f"Saving {self._file_path}, data_len={len(self._data_list)}, save_len={do_save.sum()}"
            )
                ...
                if do_save[idx]:
                    ...
                    if last_value is not None:
                        if idx == len(self._data_list) - 1:
                            ...
                            next_value = self._data_list[idx + 1]["supervision"][
                                "value"
                            ]
                        data["supervision"]["advantage"] = (
                            data["reward"] + next_value - data["supervision"]["value"]
                        )
                    ...
                    # supervision
                    self._write_dict_to_group(
                        group_step, "supervision", data["supervision"]
                    )

                    # control_diff
                    if data["control_diff"] is not None:
                        group_step.create_dataset(
                            "control_diff", data=data["control_diff"]
                        )
