...
class LbcRoamingAgent(object):
    ...
    def run_step(self, input_data, timestamp):
        ...
        hazard_vehicle_loc = lbc_hazard_vehicle(
            input_data["surrounding_vehicles"], input_data["speed"]["speed_xy"][0]
        )
    ...
    def render(self, reward_debug, terminal_debug):
        ...
        im = cv2.putText(
            im, txt_1, (2, 12), cv2.FONT_HERSHEY_SIMPLEX, 0.33, (255, 255, 255), 1
        )

        throttle, steer, brake = self._render_dict["control"]
        txt_2 = f"s:{steer:5.2f}, t:{throttle:5.2f}, b:{brake:5.2f}"
        im = cv2.putText(
            im, txt_2, (2, 24), cv2.FONT_HERSHEY_SIMPLEX, 0.33, (255, 255, 255), 1
        )
        ...
        im = cv2.putText(
            im, txt_3, (2, 36), cv2.FONT_HERSHEY_SIMPLEX, 0.33, (255, 255, 255), 1
        )

        imu_str = np.array2string(
            self._render_dict["gnss"]["imu"][:-1],
            precision=1,
            separator=",",
            suppress_small=True,
        )
        im = cv2.putText(
            im, imu_str, (w, 12), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (255, 255, 255), 1
        )

        for i, txt in enumerate(
            reward_debug["debug_texts"] + terminal_debug["debug_texts"]
        ):
            im = cv2.putText(
                im,
                txt,
    ...
    def draw_gnss(rendered_birdview, render_dict, obs_cfg):
        ...
        compass = (
            0.0
            if np.isnan(render_dict["gnss"]["imu"][-1])
            else render_dict["gnss"]["imu"][-1]
        )

        gps_point = render_dict["gnss"]["target_gps"]
        target_vec_in_global = gps_util.gps_to_location(
            gps_point
        ) - gps_util.gps_to_location(ev_gps)
        ...
        ev_xy = (
            int(birdview_cfg["width_in_pixels"] / 2),
    ...
    def draw_route(render_dict, obs_cfg):
        ...
        for i, loc in enumerate(render_dict["route_plan"]):
            x = int(
                np.round(
                    birdview_cfg["width_in_pixels"] / 2
                    + loc[1] * birdview_cfg["pixels_per_meter"]
                )
            )
            y = int(
                np.round(
                    birdview_cfg["width_in_pixels"]
                    - birdview_cfg["pixels_ev_to_bottom"]
