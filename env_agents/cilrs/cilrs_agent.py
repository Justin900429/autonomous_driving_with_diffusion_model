...
class CilrsAgent:
            ...
            if cfg.rl_run_path is not None:
                ...
                if cfg.rl_ckpt_step is None:
                    ...
                    f = min(
                        all_ckpts,
                        key=lambda x: abs(
                            int(x.name.split("_")[1].split(".")[0]) - rl_ckpt_step
                        ),
                    )
                    self._logger.info(
                        f"Download rl checkpoint closest to step {rl_ckpt_step}: {f.name}"
                    )
                f.download(replace=True)
                self._logger.info(f"Downloading {f.name}")
                rl_ckpt = f.name
        ...
        if self._ckpt is None:
            ...
            if self._env_wrapper.view_augmentation:
                im_shape = (
                    3,
                    n_stack,
                    self._obs_configs["central_rgb"]["height"],
    ...
    def run_step(self, input_data, timestamp):
        ...
        for im_key in self._im_queue.keys():
            ...
            input_data[im_key] = [
                copy.deepcopy(self._im_queue[im_key][i])
                for i in self._env_wrapper.im_stack_idx
            ]
