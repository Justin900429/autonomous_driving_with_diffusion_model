                        ...
                        for obs_key in self._obs_keys_to_load:
                            if "rgb" in obs_key:
                                obs_dict[obs_key] = [
                                    ...
                                    for i in im_stack_idx_list
                                ]

                                group_step["obs"][obs_key]
                            else:
                                obs_dict[obs_key] = self.read_group_to_dict(
                                    group_step["obs"][obs_key],
                                    [h5_path, step_str, "obs", obs_key],
                                )

                        supervision_dict = self.read_group_to_dict(
                            group_step["supervision"],
                ...
                if "rgb" in obs_key:
                    for i in range(len(obs_dict)):
                        obs[obs_key][i]["data"] = self._im_augmenter(
                            self._batch_read_number
                        ).augment_image(obs[obs_key][i]["data"])
...
def get_dataloader(
    ...
    def make_dataset(list_expert_h5, list_dagger_h5, is_train):
        ...
        dataset = CilrsDataset(
            list_expert_h5, list_dagger_h5, env_wrapper, im_augmenter
        )
    ...
    log.info(
        f"TRAIN expert episodes: {len(list_expert_h5_train)}, DAGGER episodes: {len(list_dagger_h5_train)}, "
        f"expert hours: {train_expert_frames/10/3600:.2f}, DAGGER hours: {train_dagger_frames/10/3600:.2f}."
    )
    log.info(
        f"VAL expert episodes: {len(list_expert_h5_val)}, DAGGER episodes: {len(list_dagger_h5_val)}, "
