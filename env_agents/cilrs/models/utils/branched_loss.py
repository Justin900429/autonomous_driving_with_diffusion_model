...
class BranchedLoss:
    ...
    def forward(self, outputs, supervisions, commands):
        ...
        if "action_branches" in outputs:
            ...
            for i in range(self.n_branches):
                ...
                for j in range(self.n_actions):
                    ...
                    action_loss += (
                        loss_ij * self.branch_weights[i] * self.action_weights[j]
                    )
        else:
            if self.action_kl:
                ...
                for i in range(self.n_branches):
                    dist_sup = Beta(
                        supervisions["action_mu"], supervisions["action_sigma"]
                    )
                    dist_pred = Beta(
                        outputs["mu_branches"][i], outputs["sigma_branches"][i]
                    )
                    kl_div = (
                        th.distributions.kl_divergence(dist_sup, dist_pred)
                        * branch_masks[i]
                    )
            ...
            if self.action_agg is not None:
                ...
                for i in range(self.n_branches):
                    ...
                    aggrevated_loss = (
                        -1.0
                        * th.exp(log_prob - log_prob.detach())
                        * supervisions["advantage"].unsqueeze(1)
                    )
        ...
        if "pred_speed" in outputs and "speed" in supervisions:
            speed_loss = (
                self.loss(outputs["pred_speed"], supervisions["speed"])
                * self.speed_weight
            )
        ...
        if "pred_value" in outputs and "value" in supervisions:
            value_loss = (
                F.mse_loss(outputs["pred_value"], supervisions["value"])
                * self.value_weight
            )
        ...
        if "pred_features" in outputs and "features" in supervisions:
            features_loss = (
                F.mse_loss(outputs["pred_features"], supervisions["features"])
