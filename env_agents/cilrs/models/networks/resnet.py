...
class ResNet(nn.Module):
    def __init__(self, block, layers, input_shape, num_classes=1000):
        ...
        self.conv1 = nn.Conv2d(
            im_channels, 64, kernel_size=7, stride=2, padding=3, bias=False
        )
...
def get_model(model_name, input_shape, num_classes, pretrained=False):
    ...
    if model_name == "resnet34_cilrs":
        ...
        if im_channels != 3:
            ...
            print(
                f"Mismatch {model_name} first conv input channel. desired:{im_channels}, predefined:3"
            )
            old = model.conv1
        ...
        if num_classes != model.fc.out_features:
            print(
                f"Mismatch {model_name} last fc output dim. desired:{num_classes}, predefined:{model.fc.out_features}"
            )
