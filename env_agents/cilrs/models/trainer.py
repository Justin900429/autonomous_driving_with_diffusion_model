...
class Trainer:
    ...
    def learn(self, dataset_dir, train_epochs, env_wrapper, reset_step=False):
        ...
        for idx_epoch in range(self.starting_epoch, train_epochs):
            ...
            log.info(
                f"Epoch {idx_epoch}: {(time.time()-t0)/3600:.2f} hours, "
    ...
    def _train(self, dataset):
        ...
        for command, policy_input, supervision in dataset:
            ...
            supervision = dict(
                [(k, th.as_tensor(v).to(self.device)) for k, v in supervision.items()]
            )
        ...
        for command, policy_input, supervision in dataset:
            ...
            with th.no_grad():
                ...
                (
                    ...
                    value_loss,
                    features_loss,
                ) = self.criterion.forward(outputs, supervision, command)
                loss = action_loss + speed_loss + value_loss + features_loss
