...
class CoILICRA(nn.Module):
    def __init__(
        ...
        input_states,
        acc_as_action,
        value_as_supervision,
        action_distribution,
        dim_features_supervision,
        rl_ckpt=None,
        ...
        if rl_ckpt is not None:
            try:
                rl_state_dict = th.load(rl_ckpt, map_location="cpu")[
                    "policy_state_dict"
                ]
        ...
        self.perception = resnet.get_model(
            perception_net,
            im_shape,
            num_classes=perception_output_neurons,
            pretrained=resnet_pretrain,
        )
        ...
        self.measurements = FC(
            params={
                "neurons": [input_states_len] + measurements_neurons,
        ...
        self.join = Join(
            params={
                "after_process": FC(
                    params={
                        "neurons": [
                            measurements_neurons[-1] + perception_output_neurons
                        ]
                        + join_neurons,
        ...
        self.speed_branch = FC(
            params={
                "neurons": [perception_output_neurons] + speed_branch_neurons + [1],
        ...
        if value_as_supervision:
            self.value_branch = FC(
                params={
                    "neurons": [join_neurons[-1]] + value_branch_neurons + [1],
        ...
        if action_distribution == "beta":
            ...
            for i in range(number_of_branches):
                mu_branch_vector.append(
                    FC(
                        params={
                            "neurons": [join_neurons[-1]]
                            + branches_neurons
                            + [dim_out],
            ...
            for i in range(number_of_branches):
                policy_head = FC(
                    params={
                        "neurons": [join_neurons[-1]] + branches_neurons,
                        "dropouts": branches_dropouts,
                        "end_layer": nn.ReLU,
                    }
                )
                dist_mu = nn.Sequential(
                    nn.Linear(branches_neurons[-1], dim_out), nn.Softplus()
                )
                ...
                if rl_state_dict is not None:
                    ...
                    log.info(
                        f"Load rl_ckpt state dict for policy_head, dist_mu, dist_sigma."
                    )
    ...
    def forward_branch(self, command, im, state):
        with th.no_grad():
            ...
            if (
                self.action_distribution == "beta"
                or self.action_distribution == "beta_shared"
            ):
                action_branches = self._get_action_beta(
                    outputs["mu_branches"], outputs["sigma_branches"]
                )
    ...
    def extract_branch(action_branches, branch_number):
        ...
        branch_number = th.stack(
            [branch_number, th.LongTensor(range(0, len(branch_number)))]
        )
    ...
    def _load_state_dict(il_net, rl_state_dict, key_word):
        ...
        assert len(rl_keys) == len(
            il_net.state_dict().keys()
        ), f"mismatch number of layers loading {key_word}"
