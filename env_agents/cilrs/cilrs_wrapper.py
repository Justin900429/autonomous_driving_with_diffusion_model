...
class CilrsWrapper:
    def __init__(
        ...
        self._im_transform = T.Compose(
            [T.ToTensor(), T.Normalize(mean=im_mean, std=im_std)]
        )
    ...
    def process_supervision(self, supervision):
        ...
        if self.action_distribution is not None:
            ...
            processed_supervision["action_sigma"] = th.tensor(
                supervision["action_sigma"]
            )
        ...
        processed_supervision["speed"] = (
            th.tensor(supervision["speed"]) / self.speed_factor
        )
    ...
    def im_render(render_dict):
        ...
        state_str = np.array2string(
            render_dict["policy_input"]["state"].numpy(),
            precision=2,
            separator=",",
            suppress_small=True,
        )
        ...
        im = cv2.putText(
            im, txt_1, (w, 12), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (255, 255, 255), 1
        )
        txt_2 = f'cmd: {render_dict["command"][0]} s{state_str}'
        im = cv2.putText(
            im, txt_2, (w, 24), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (255, 255, 255), 1
        )
