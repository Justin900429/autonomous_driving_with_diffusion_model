...
class WandbCallback(BaseCallback):
        ...
        wandb.log(
            {
                ...
                "time/fps": (self.model.num_timesteps - self.model.start_num_timesteps)
                / time_elapsed,
    ...
    def _on_rollout_end(self):
        ...
        for i in range(n_action):
            ...
            wandb.log(
                {f"alpha[{i}]": wandb.Histogram(mu_statistics[:, i])},
                step=self.model.num_timesteps,
            )
            wandb.log(
                {f"beta[{i}]": wandb.Histogram(sigma_statistics[:, i])},
                step=self.model.num_timesteps,
            )
        ...
        if (self.model.num_timesteps - self._last_time_buffer) >= self._buffer_step:
            self._last_time_buffer = self.model.num_timesteps
            buffer_video_path = (
                self._video_path / f"buffer_{self.model.num_timesteps}.mp4"
            ).as_posix()
            ...
            wandb.log(
                {f"buffer/{self.model.num_timesteps}": wandb.Video(buffer_video_path)},
                step=self.model.num_timesteps,
            )
